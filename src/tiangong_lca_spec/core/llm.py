...
class LanguageModelProtocol(Protocol):
    """Minimal protocol implemented by language-model clients within the workflow."""

    def invoke(self, input_data: dict[str, Any]) -> Any: ...


class OpenAIResponsesLLM(LanguageModelProtocol):
    """Thin wrapper around the OpenAI Responses API with optional disk caching."""
...
def load_openai_credentials(path: Path) -> tuple[str, str]:
    """Load OpenAI API credentials (api_key, model) from the secrets TOML file."""
    parsed = tomllib.loads(path.read_text(encoding="utf-8"))
    openai_cfg = parsed.get("openai", {})
